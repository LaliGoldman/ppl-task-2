...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | DictExp; // <-- added DictExp to CompoundExp
...
export type DictExp = { tag: "DictExp"; entries: [string, CExp][]; } //  <-- definde DictExp
...
export const makeDictExp = (entries: [string, CExp][]): DictExp =>
    ({ tag: "DictExp", entries });

// export const makeDictExp = (entries: [SymbolSExp, CExp][]): DictExp => 
//     ({tag: "DictExp", entries: entries.map(([k, v]) => [k.val, v])});  // <-- NEW CONSTRUCTOR - makeDictExp
...
export const isDictExp = (x: any): x is DictExp => x.tag === "DictExp";       // <-- NEW PREDICATE
...
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x) || isDictExp(x);  // <-- include DictExp here
...
// CompoundCExp -> IfExp | ProcExp | LetExp | LitExp | AppExp | DictExp // <-- added DictExp
...
export const parseL32SpecialForm = (op: Sexp, params: Sexp[]): Result<CExp> =>
    ...
    op === "dict"   ? 
        parseDictExp(params) :   // ← add this line
        makeFailure("Never");

        
    
    export const hasDuplicateKeys = (keys: string[]): boolean =>
        new Set(keys).size !== keys.length;
      
    export const parseDictExp = (params: Sexp[]): Result<DictExp> => {
        if (!allT(isNonEmptyList, params) || params.some(p => (p as Sexp[]).length !== 2)) {
          return makeFailure(`Bad dict entries: ${format(params)}`);
        }
      
        const parsedPairsResult = mapResult(
          (p: Sexp): Result<[string, CExp]> => {
            const pair = p as Sexp[];
            return bind(parseL32CExp(pair[0]), k2 => {
              const keyStr =
                isLitExp(k2) && isSymbolSExp(k2.val)
                  ? k2.val.val
                  : isVarRef(k2)
                  ? k2.var
                  : undefined;
              ...
              return keyStr === undefined
                ? makeFailure(`Dict key must be a symbol or variable: ${format(pair[0])}`)
                : mapv(parseL32CExp(pair[1]), v2 =>
                    [keyStr, v2] as [string, CExp]);
            });
          },
          params
        );
      
        return bind(parsedPairsResult, (pairs: [string, CExp][]) => {
          return hasDuplicateKeys(pairs.map(([k]) => k))
            ? makeFailure(`Duplicate keys in dict: ${format(pairs.map(([k]) => k))}`)
    ...
    // export const parseDictExp = (params: Sexp[]): Result<DictExp> =>
    //     (!allT(isNonEmptyList, params) ||
    //      params.some(p => (p as Sexp[]).length !== 2))
    //       ? makeFailure(`Bad dict entries: ${format(params)}`)
    //       : mapv(
    //           mapResult(
    //             (p: Sexp): Result<[string, CExp]> => {
    //               const pair = p as Sexp[];
    ...
    //             params
    //           ),
    //           (pairs: [string, CExp][]) => makeDictExp(pairs)
    //         );
...
const unparseDictExp = (de: DictExp): string =>                // <-- Dict UNPARSER
    `(dict ${de.entries.map(([k,v]) => `(${unparseL32(makeLitExp(makeSymbolSExp(k)))} ${unparseL32(v)})`).join(" ")})`;
...
export const unparseL32 = (exp: Program | Exp): string =>
    ...
    isDictExp(exp)    ? unparseDictExp(exp) :       // <-- NEW UNPARSER
